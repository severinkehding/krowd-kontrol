...
public:
	...
	// that run the corridor's full length, flanking ConnectorFloorMeshComponent on
	// either side, so the player can't drift sideways off the connector strip. Kept
	// independently tunable rather than reading ARoomActor's RoomWallHeight/
	// RoomWallThickness - same reasoning ConnectorFloorThickness/DoorMarkerHeight
	// already use (a door can connect two rooms with different wall dimensions).
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Door Connector")
