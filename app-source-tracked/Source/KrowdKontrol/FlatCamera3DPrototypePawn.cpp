...
void AFlatCamera3DPrototypePawn::CastSleepAbility()
{
	...
	else
	{
		...
		// press. Matches this codebase's existing "never leave an ability press
		// completely unhandled" precedent. Logged (PR #280 review, LOW finding 1)
		// since this silently downgrades Sleep from AoE to single-target for the
		// press, which would otherwise be undiagnosable from a log pull alone.
		UE_LOG(LogTemp, Warning, TEXT("AFlatCamera3DPrototypePawn::CastSleepAbility: no cursor world position this frame, falling back to single-target auto-nearest cast"));
...
void AFlatCamera3DPrototypePawn::CastRootAbility()
{
	...
	if (GetCursorWorldPosition(CursorWorldPosition))
	{
		AbilityPressHoldComponent->HandleAbilityKeyPressed(EAbilitySlot::Root, true, CursorWorldPosition);
	}
	else
	{
		// No live viewport / possessing controller this frame (see
		// GetCursorWorldPosition()'s own doc comment) - fall back to the
		// pre-cursor auto-nearest-target path rather than silently dropping the
		// press, same precedent CastStunAbility()/CastSleepAbility() establish.
		// Logged since this silently downgrades Root from a cursor-aimed line to
		// single-target for the press, which would otherwise be undiagnosable
		// from a log pull alone.
		UE_LOG(LogTemp, Warning, TEXT("AFlatCamera3DPrototypePawn::CastRootAbility: no cursor world position this frame, falling back to single-target auto-nearest cast"));
...
void AFlatCamera3DPrototypePawn::CastSnareAbility()
{
	...
	if (GetCursorWorldPosition(CursorWorldPosition))
	{
		AbilityPressHoldComponent->HandleAbilityKeyPressed(EAbilitySlot::Snare, true, CursorWorldPosition);
	}
	else
	{
		// No live viewport / possessing controller this frame (see
		// GetCursorWorldPosition()'s own doc comment) - fall back to the
		// pre-cursor auto-nearest-target path rather than silently dropping the
		// press, same precedent CastStunAbility()/CastSleepAbility()/CastRootAbility()
		// establish. Logged since this silently downgrades Snare from a cursor-aimed
		// cone to single-target for the press, which would otherwise be
		// undiagnosable from a log pull alone.
		UE_LOG(LogTemp, Warning, TEXT("AFlatCamera3DPrototypePawn::CastSnareAbility: no cursor world position this frame, falling back to single-target auto-nearest cast"));
