...
public:
	...
	// zoom-out retuned by issue #251, PRD "Mission Briefing & Live Quest Tracker"
	// REQ-4, after 2026-08-22 playtests found the shipped default too close for
	// crowd management) - EditAnywhere so designers can retune "feels far away /
	// hard to read" without a C++ recompile. Ranges below are the newly documented
	// defaults' valid bounds; KrowdKontrol.Unit.FlatCamera3DPipelineCameraFraming
	// asserts both the bounds and that these properties genuinely drive
	// CameraBoom/TopDownCamera via ApplyCameraFraming() below, not just at
	// construction time.
