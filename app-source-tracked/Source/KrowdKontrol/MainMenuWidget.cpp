...
void UMainMenuWidget::HandleLevelSelected(FName MapName)
{
	...
	{
		// Level 1 enters through the intro flythrough (operator scene brief,
		// 2026-09-02): L_Intro's AIntroFlythroughDirector travels on to
		// L_Level01 itself after its 7-second flight. Every other level opens
		// directly - the intro is the game's opening, not a per-level stinger.
		// LastSelectedLevelMapName above stays the REAL selected level either
		// way, preserving the seam the menu tests pin.
		const FName TravelMapName = MapName == FName(TEXT("L_Level01"))
			? FName(TEXT("L_Intro")) : MapName;
