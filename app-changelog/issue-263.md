...
`GetCursorWorldPosition()` each frame and, on success, delegates to a new
`ApplyFacingTowardCursor()` method, which computes a yaw-only `FRotator` via a new
pure static helper `ComputeFacingRotation()` (mirroring
...
`SetActorRotation()`. `ApplyFacingTowardCursor()` is split out from `Tick()`
specifically so the real apply-rotation wiring is exercisable with a fixed cursor
position, without needing a live viewport to reach it through `Tick()`'s own
`GetCursorWorldPosition()` call (review follow-up on PR #279). `MoveForward`/
`MoveRight` are untouched - they already use fixed world axes
(`FVector::ForwardVector`/`FVector::RightVector`), not actor-relative ones, so WASD
movement stays rotation-independent by construction.
...
- [x] Automation test: given a fixed cursor world position, the computed facing
      ...
      `KrowdKontrol.Unit.FlatCamera3DPrototypePawnFacingRotationMath` (6 cases:
      axis-aligned (+X/+Y/-X/-Y), off-axis, degenerate).
- [x] Automation test: the real `ComputeFacingRotation() -> SetActorRotation()`
      apply-rotation wiring runs end-to-end for a fixed cursor position, not just
      its two halves in isolation - covered by
