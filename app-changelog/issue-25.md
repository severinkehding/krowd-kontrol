...
| `Private/Tests/KrowdKontrolMusicSubsystemTest.cpp` | CREATE | `KrowdKontrol.Unit.MusicSubsystem`, cases (a)-(j): default state, single-enemy full Idle->Alert->Attack->Controlled->Banked cycle, idempotent no-op refresh, two-enemy aggregation, flip-before-broadcast re-entrancy assertion, a real-track crossfade case asserting `CurrentMusicComponent` actually spawns/replaces a `UAudioComponent`, a Config-driven shipping-asset-path case (k) kept in sync with `DefaultGame.ini`, and a natural-reversion case (l) proving destroying the only Hot enemy reverts to Calm (the enemy state machine has no Alert->Idle de-detection yet, so destruction and banking are the only runtime reversion paths - documented in the test). |
| `Private/Tests/KrowdKontrolEnemyBaseTest.cpp` | UPDATE | Added cases (m)/(n): `FindPlayerEnergyComponent()`'s found and not-found paths. **Deliberately retained by operator decision (2026-08-17)** despite originating in issue #15's scope: the method itself now lives on `main` (landed via PR #119), these cases are the only automation coverage it has, and extra passing coverage is welcome - reviewers should treat this as an accepted, documented scope exception, not an oversight. |
