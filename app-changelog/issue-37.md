...
| `app/Source/KrowdKontrol/Private/Tests/KrowdKontrolAbilityMatchupSignalComponentTest.cpp` | CREATE | `KrowdKontrol.Unit.AbilityMatchupSignalComponent` — 6 cases: matched cast, mismatched cast, Stun-never-matched (against `ARunnerEnemy`/RU_NNR, the one type that would otherwise collide with Stun's default `CounteredEnemyType`, so the case actually depends on the `bIsColourNeutral` short-circuit), missing-indicator defensive, real-pawn constructor wiring, null-target defensive |
