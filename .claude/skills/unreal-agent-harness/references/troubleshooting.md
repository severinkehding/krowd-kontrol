...
- **A file sitting in `Content/` triggers Unreal's own file-watcher import dialog — separate from, and in addition to, `StaticMeshTools.import_file`.** The MCP tool's underlying `AssetImportTask.automated = True` (confirmed by reading the plugin's Python source, `EditorToolset/Content/Python/editor_toolset/toolsets/asset.py`) already suppresses the *import-options* dialog for the MCP-driven import itself. But if the source FBX/OBJ was written directly under the project's `Content/` folder (e.g. by a Blender export that lands there), Unreal's content-browser file-watcher notices the new file independently and pops its own "import this?" prompt — genuinely not headless, needs a manual click. **Fix: export/stage the source file *outside* `Content/`** (anywhere on disk — `source_file` doesn't need to be inside the project) and only bring it in via `import_file`. Nothing to click.
- **"No smoothing group information was found in this FBX scene" (Unreal import warning).** Blender's `bpy.ops.export_scene.fbx()` defaults `mesh_smooth_type='OFF'` — no smoothing data at all. Pass `mesh_smooth_type='FACE'` on export to fix it at the source. Confirmed via `EditorToolset.LogsToolset.GetLogEntries` (pattern `FBXImport`): the warning disappeared entirely on the next import once the export was regenerated with that flag set.
